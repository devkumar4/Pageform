        ...
        >
          ...
          {droppable.isOver && elements.length === 0 && (
            <div className="p-4 w-full">
              <div className="h-[120px] rounded-mg bg-primary/20"></div>
            </div>
          )}
          {elements.length > 0 && (
...
function DesignerElementWrapper({ element }: { element: FormElementInstance }) {
  ...
  return (
    <div
      ...
      className="relative h-[120px] flex flex-col text-foreground hover:cursor-pointer rounded-md ring-accent ring-inset"
      onMouseEnter={() => {
        setMouseIsOver(true);
      }}
      onMouseLeave={() => {
        ...
        <>
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 animate-pulse">
            <p className="text-muted-foreground text-sm">
              Clilk for properties or drag to move
            </p>
          </div>
        </>
      )}
